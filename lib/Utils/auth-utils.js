...
const serial_task_queue_1 = require("./serial-task-queue");
/**
    ...
    return {
        async get(type, ids) {
            return cacheMutex.runExclusive(async () => {
                const data = {};
                const idsToFetch = [];
                const items = await cache.mget?.(ids.map(id => getUniqueId(type, id)));
                if (items) {
                    for (const id of ids) {
...
const addTransactionCapability = (state, logger, { maxCommitRetries, delayBetweenTriesMs }) => {
    ...
    function getQueue(key) {
        if (!keyQueues.has(key)) {
            keyQueues.set(key, new serial_task_queue_1.SerialTaskQueue());
        }
