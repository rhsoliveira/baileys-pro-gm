...
const payment_order_js_1 = require("./payment-order.js");
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('buttons' in message && !!message.buttons) {
        const buttonsMessage = {
            buttons: message.buttons.map(b => ({
                ...b,
                type: index_js_1.proto.Message.ButtonsMessage.Button.Type.RESPONSE
            }))
        };
        if ('text' in message) {
            //@ts-ignore
            ...
            if (Object.keys(m)?.length) {
                ...
                const type = firstKey ? firstKey.replace('Message', '').toUpperCase() : undefined;
    ...
    if ('orderChangeStatus' in message && !!message.orderChangeStatus) {
        const { status, referenceId } = message.orderChangeStatus;
        const orderChangeStatus = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: [
                    {
                        name: Types_1.WAWebInteractiveMessagesNativeFlowNameEnum.REVIEW_ORDER,
        ...
        m = { interactiveMessage: orderChangeStatus };
    }
    if ('orderPaymentChangeStatus' in message && !!message.orderPaymentChangeStatus) {
        const { status, referenceId } = message.orderPaymentChangeStatus;
        const orderChangeStatus = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: [
                    {
                        name: Types_1.WAWebInteractiveMessagesNativeFlowNameEnum.PAYMENT_STATUS,
                        buttonParamsJson: JSON.stringify({
                            reference_id: referenceId,
    ...
    if ('paymentOrder' in message && !!message.paymentOrder) {
        const items = message.paymentOrder.items || [];
        const hasReferral = !!message.paymentOrder.referral;
        let paramsJson = {
            total_amount: {
                offset: 1,
                value: message.paymentOrder.amount || 0
            }
        };
        // If there is no referral, validate items and total amount
        if (!hasReferral) {
            if (items.length === 0) {
                throw new boom_1.Boom('At least one item is required in payment order', { statusCode: 400 });
            }
            const formattedItems = items.map(e => {
                return {
                    ...e,
                    retailer_id: e.retailer_id || payment_order_js_1.PaymentOrderFunctions.generateCustomRetailerId()
                };
            });
            const totalAmount = formattedItems.reduce((acc, item) => acc + item.amount.value * item.quantity, 0);
            if (totalAmount < 1 || totalAmount > 5000) {
                throw new boom_1.Boom('Total amount must be between 1 and 5000', { statusCode: 400 });
            }
            paramsJson = {
                total_amount: {
                    offset: 1,
        ...
        const cardItem = message.paymentOrder.paymentSettings.find(e => e.type === 'cards');
        const othersPayments = message.paymentOrder.paymentSettings.filter(e => ['boleto'].includes(e.type));
        if (hasReferral && (cardItem?.cards.enabled || othersPayments.length > 0)) {
            throw new boom_1.Boom('Referral cannot be used with cards enabled or boleto payment methods', { statusCode: 400 });
        }
        const paymentOrderMessage = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: [
                    {
                        name: Types_1.WAWebInteractiveMessagesNativeFlowNameEnum.REVIEW_AND_PAY,
                        buttonParamsJson: JSON.stringify({
                            type: 'physical-goods',
                            additional_note: '',
                            payment_settings: message.paymentOrder.paymentSettings,
                            reference_id: message.paymentOrder.referenceId || payment_order_js_1.PaymentOrderFunctions.generateReferenceId(),
                            currency: message.paymentOrder.currency || 'BRL',
                            ...paramsJson
                        })
                    }
                ]
        ...
        m = { interactiveMessage: paymentOrderMessage };
    }
    if ('sections' in message && !!message.sections) {
        const listMessage = {
            ...
            listType: message.hasOwnProperty('listType') ? message.listType : index_js_1.proto.Message.ListMessage.ListType.PRODUCT_LIST
        };
        m = { listMessage };
    }
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        const interactiveMessage = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.create({
    ...
    if ('cards' in message && !!message.cards) {
        const slides = await Promise.all(message.cards.map(async (slide) => {
            const { image, video, body, footer, buttons } = slide;
            let header = {};
            // if (product) {
            ...
            // 			product: {
            // 				...product,
            // 				...(imageMessage && { productImage: imageMessage as any })
            // 			},
            // 			...slide
            // 		}
            // 	})
