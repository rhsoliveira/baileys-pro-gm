...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (additionalNodes && additionalNodes.length > 0) {
                ...
                if (buttonType) {
                    ;
                    stanza.content.push(getButtonArgs(message));
    ...
    const getButtonType = (message) => {
        ...
        else if (message.buttonsMessage || message.documentWithCaptionMessage?.message?.buttonsMessage) {
            return 'buttons';
        }
        else if (message.buttonsResponseMessage) {
            return 'buttons_response';
    ...
    const getButtonArgs = (message) => {
        // Payment Orders - Native Flows
        const data = {
            [Types_1.WAWebInteractiveMessagesNativeFlowNameEnum.REVIEW_AND_PAY]: 'order_details',
            [Types_1.WAWebInteractiveMessagesNativeFlowNameEnum.REVIEW_ORDER]: 'order_status',
            [Types_1.WAWebInteractiveMessagesNativeFlowNameEnum.PAYMENT_STATUS]: 'payment_status'
        };
        ...
        if (message.interactiveMessage?.nativeFlowMessage &&
            ...
            firstButtonName &&
            data[firstButtonName]) {
            const name = data[firstButtonName];
            return {
                tag: 'biz',
                attrs: {},
                content: [
        ...
        // 						attrs: {
        // 							v: '2',
        // 							name: WAInteractiveMessagesNativeFlowName.QUICK_REPLY
        // 						}
        // 					}
        // 				]
        // 			}
        ...
        // 	}
        // }
        if (message?.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name ===
            Types_1.WAInteractiveMessagesNativeFlowName.PAYMENT_INFO) {
            baseAttrs.content = [
                {
                    tag: 'interactive',
                    attrs: {
                    ...
                    content: [
                        {
                            ...
                            attrs: {
                                v: '2',
                                name: Types_1.WAInteractiveMessagesNativeFlowName.MIXED
                            }
                        }
                    ]
                }
